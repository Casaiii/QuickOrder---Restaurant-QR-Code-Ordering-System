  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          className="blue-gradient text-white hover:scale-105 transition-transform shadow-lg rounded-full px-6"
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto bg-gradient-to-br from-white to-blue-50/30">
        ...
        <div className="space-y-6">
          <Card className="premium-card">
            <CardContent className="p-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-bold blue-gradient bg-clip-text text-transparent">
                  NT$ {menuItem.price}
                </span>
              </div>
            </CardContent>
          ...
          {menuItem.customizations &&
            menuItem.customizations.map((customization) => (
              <Card key={customization.id} className="premium-card">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg flex items-center gap-2">
                    {customization.name}
                    {customization.required && <Badge className="blue-gradient text-white text-xs">必選</Badge>}
                  </CardTitle>
                </CardHeader>
                  ...
                  {customization.type === "single" ? (
                    ...
                    >
                      {customization.options.map((option) => (
                        <div
                          key={option.id}
                          className="flex items-center space-x-3 p-3 rounded-lg hover:bg-blue-50/50 transition-colors"
                        >
                          ...
                          >
                            ...
                            {option.price > 0 && (
                              <span className="text-blue-600 font-semibold">+NT$ {option.price}</span>
                            )}
                          </Label>
                        </div>
                      ))}
          ...
          <Card className="premium-card border-blue-200">
            <CardContent className="p-6">
              <div className="flex justify-between items-center mb-4">
                <span className="text-lg font-semibold text-gray-900">總價格</span>
